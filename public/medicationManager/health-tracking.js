class HealthTrackingManager {
    ...
    bindEvents() {
        ...
        // modal events
        document.getElementById('addMetricBtn').addEventListener('click', () => this.openMetricModal());
        document.getElementById('closeMetricModal').addEventListener('click', () => this.closeMetricModal());
        document.getElementById('cancelMetricBtn').addEventListener('click', () => this.closeMetricModal());
        document.getElementById('metricForm').addEventListener('submit', (e) => this.handleMetricSubmit(e));

        // generate report
        document.getElementById('generateReportBtn').addEventListener('click', () => this.generateHealthReport());

        // metric type change for units
        document.getElementById('metricType').addEventListener('change', (e) => this.updateUnitPlaceholder(e.target.value));
        ...
        document.getElementById('metricDateTime').value = new Date().toISOString().slice(0, 16);
    }
    ...
    initComplianceChart() {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const avgCompliance = this.complianceData.length > 0 ? 
                this.complianceData.reduce((acc, med) => acc + med.complianceRate, 0) / this.complianceData.length : 0;
            complianceRates.push(Math.max(0, avgCompliance + (Math.random() - 0.5) * 20));
        }
    ...
    initTrendsChart() {
        ...
        const metricTypes = [...new Set(this.healthMetrics.map(m => m.metricType))];
        const datasets = metricTypes.slice(0, 3).map((type, index) => {
            const typeMetrics = this.healthMetrics.filter(m => m.metricType === type);
    ...
    renderMedicationCompliance() {
        ...
        container.innerHTML = this.complianceData.map(med => {
            const complianceClass = med.complianceRate >= 80 ? 'compliance-high' : 
                                  med.complianceRate >= 60 ? 'compliance-medium' : 'compliance-low';
            
            return `
                <div class="border border-gray-200 rounded-lg p-4">
                    <div class="flex justify-between items-center mb-2">
                        <h4 class="font-semibold text-gray-800">${this.escapeHtml(med.name)}</h4>
                        <span class="text-2xl font-bold ${med.complianceRate >= 80 ? 'text-green-600' : 
                                                        med.complianceRate >= 60 ? 'text-yellow-600' : 'text-red-600'}">
    ...
    renderHealthMetrics() {
        ...
        container.innerHTML = Object.entries(groupedMetrics).map(([type, metrics]) => {
            ...
            if (previous && parseFloat(latest.value) !== parseFloat(previous.value)) {
                    ...
                    <div class="border border-gray-200 rounded-lg p-4">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <span class="px-2 py-1 rounded text-xs font-medium ${
                                ...
                                item.trend === 'concerning' ? 'bg-red-100 text-red-800' :
    ...
    analyzeTrends() {
        ...
        return Object.entries(grouped).map(([metricType, data]) => {
            const avgValue = data.reduce((sum, item) => sum + item.avgValue, 0) / data.length;
            const minValue = Math.min(...data.map(item => item.minValue));
            const maxValue = Math.max(...data.map(item => item.maxValue));
            
            // simple trend analysis
            let trend = 'stable';
            let description = 'Values have remained relatively stable over the observed period.';
            ...
            if (data.length >= 3) {
                ...
                if (Math.abs(changePercent) > 10) {
                    if ((metricType === 'weight' && changePercent < 0) || 
                        (metricType === 'blood_pressure' && changePercent < 0) ||
                        (metricType === 'heart_rate' && changePercent > -5 && changePercent < 5)) {
                        trend = 'improving';
    ...
    updateOverviewCards() {
        ...
        const overallCompliance = this.complianceData.length > 0 ? 
            Math.round(this.complianceData.reduce((acc, med) => acc + med.complianceRate, 0) / this.complianceData.length) : 0;
        document.getElementById('overallCompliance').textContent = `${overallCompliance}%`;

        // update total medications
        document.getElementById('totalMedications').textContent = this.complianceData.length;

        // update health records count
        document.getElementById('healthRecords').textContent = this.healthMetrics.length;
    ...
    calculateStreak() {
        ...
        const highCompliance = this.complianceData.filter(med => med.complianceRate >= 80).length;
