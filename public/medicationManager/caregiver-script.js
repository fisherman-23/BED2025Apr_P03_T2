...
async function loadPatientCompliance(patientId) {
    try {
        const response = await fetch(`/api/caregiver/patients/${patientId}/dashboard`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${getAuthToken()}`
        ...
        if (response.ok) {
            ...
            const complianceElement = document.getElementById(`compliance-${patientId}`);
            const complianceBarElement = document.getElementById(`compliance-bar-${patientId}`);
            ...
            if (complianceElement && complianceBarElement) {
                ...
                if (compliance >= 90) {
                    complianceBarElement.style.background = 'linear-gradient(to right, #10b981, #34d399)';
                } else if (compliance >= 70) {
                    complianceBarElement.style.background = 'linear-gradient(to right, #f59e0b, #fbbf24)';
                } else {
                    complianceBarElement.style.background = 'linear-gradient(to right, #ef4444, #f87171)';
                }
            }
        }
    } catch (error) {
        ...
        <div class="space-y-6">
            ...
            ${dashboardData.recentMissed.length > 0 ? `
                <div class="team-card">
                    ...
                    <div class="space-y-2">
                        ${dashboardData.recentMissed.map(missed => `
