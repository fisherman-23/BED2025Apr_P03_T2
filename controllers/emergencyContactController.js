...
class EmergencyContactsController {
    ...
    try {
      ...
      if (contacts.length === 0) {
        return res.status(200).json({
          status: "info",
          message:
            "No emergency contacts available for this alert level or delay period not met",
        });
      }
      ...
      const alertPromises = contacts.map(async (contact) => {
        ...
        console.log(
          `Alert sent to ${contact.contactName} (${contact.phoneNumber}): ${alertMessage}`
        );
  ...
  async testEmergencyContact(req, res) {
    try {
      ...
      console.log(
        `Test alert sent to ${contact.contactName} (${contact.phoneNumber}): ${testMessage}`
      );
